...
def read_photo_meta(root: Path, photo):
    ...
    if is_video_suffix(photo.rel):
        # Videos: stream the sha256 (no whole-file load — multi-GB clips
        # would spike RSS); poster_qimage takes the path, not bytes.
        # probe_creation_time fills date_taken via the container metadata
        # (exiv2's video support is patchy; ini values stay in force for
        # all other fields). Applies to indexer AND backfill identically.
        try:
            st = src.stat()
            size, mtime = st.st_size, int(st.st_mtime)
            sha = _sha256_path(src)
        except OSError:
            size, mtime, sha = -1, -1, hashlib.sha256(b"").hexdigest()
...
def _index_one(root: Path, photo, idx: int, levels: list[int]):
    ...
    sha256/size/mtime identity: videos use a streaming hash (_sha256_path)
    so multi-GB clips never spike RSS; the poster decode receives the file
    PATH, not bytes (PyAV reads seekably from disk — moov-at-end MP4s
    work, and the path is reusable without reloading)."""
    ...
    if is_video:
        ...
        img = poster_qimage(root / photo.rel)
