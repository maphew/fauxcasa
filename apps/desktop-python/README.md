...
| Thumbnail cache | `thumbcache.py` | reads/builds packed fcache; machine-local under `cache/tracer-cache/` (N1/N3); identity = sha256 + relpath, staleness = size+mtime cheap signals (N6). Parallel indexer (threads + **scaled decode**) prioritizes gallery-visible photos and publishes each completed thumbnail live; a determinate activity row directly below the toolbar makes cold-build progress unmistakable — see below |
