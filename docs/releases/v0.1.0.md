...
1. Download `fauxcasa-0.1.0-rc2-windows-x64.zip` from the
...
1. Download `fauxcasa-0.1.0-rc2-linux-x64.tar.gz` from the
   [release page](https://github.com/maphew/fauxcasa/releases/tag/v0.1.0-rc2).
2. Extract it: double-click it in your file manager, or run
   `tar -xzf fauxcasa-0.1.0-rc2-linux-x64.tar.gz` in a terminal.
...
- **Star** a photo with Space. Stars you set or remove in Fauxcasa are
  ...
  forgets them. Stars Picasa recorded show up too. The **Starred Photos**
  collection groups by month, newest first, with the same pinned headers,
  jump buttons and per-group play as the main grid, so old stars never
  bury a fresh triage pass. **View > Stars** narrows any view — a folder,
  an album, a search, Starred itself — to photos with at least N stars.
  `Shift+Space` (or **View > Clear Star(s)**) clears stars on the whole
  current selection in one gesture — select-all (Ctrl+A) first to clear
  an entire folder, search or Starred scope at once.
...
- **Formats**: JPEG, PNG, GIF, BMP, TIFF, WebP and TGA stills; RAW files
  ...
  Photoshop PSD; Apple's HEIC/HEIF format (what recent iPhones save by
...
The pictures below come from a demo library, not anyone's family
archive. The photos are from [picsum.photos](https://picsum.photos)
(Unsplash licence); the small "Camera Odds and Ends" folder holds test
files from [ianare/exif-samples](https://github.com/ianare/exif-samples)
(CC BY-SA 4.0). Every name, album and face tag is made up. The images
are linked from the project repository so that this document also works
inside the downloaded program folder.

![The gallery: folders down the left with albums and people below them, photos grouped by folder on the right; stars and place markers sit on the thumbnails](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/gallery.jpg)
...
![The photo viewer showing one photo full size, with the search box still filled in above it](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/viewer.jpg)

The viewer. Double-click a photo, or select it and press Enter; Esc goes
back.

![The viewer with a white box around a face and the name Alice Hartley under it](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/faces.jpg)

Press `F` in the viewer to see the face boxes Picasa drew, with the names
you gave them.

![An album selected in the sidebar; its four photos come from two different folders](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/album.jpg)

An album from Picasa. Albums can gather photos from several folders.

![A person selected in the sidebar; the three photos they appear in, one selected, with the Info panel open on the right](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/people.jpg)

A person. Picasa's face tags become the People list, and the Info panel
shows who is in the selected photo.

![The word lake typed in the search box; the gallery shows only the matching photos](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/search.jpg)

Search as you type. This one matched a folder name and keywords at once.

![The Info panel beside the gallery, listing the selected photo's name, folder, date taken, caption, keywords, location and album](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/info.jpg)

The Info panel (`I`): date taken, caption, keywords, place and albums for
the selected photo.

![A photo filling the whole screen, with a thin caption bar at the bottom](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/slideshow.jpg)

The slideshow (the **Play** button or F11), here over the Starred view.

![The gallery at its smallest thumbnail size, dozens of photos per row](https://raw.githubusercontent.com/maphew/fauxcasa/main/docs/releases/gallery/zoomed-out.jpg)
...
| Hold in tray | `Ctrl+H` |
| Star / unstar | `Space` |
| Clear star(s) on selection | `Shift+Space` |
| Deselect | `Ctrl+D` |
...
| Pan down | `Ctrl+Down` |
| Star / unstar | `Space` |
| Clear star on this photo | `Shift+Space` |
| Next photo | `Right`, `J` |
| Previous photo | `Left`, `K` |
| Play / pause video | `P` |
...
- `config.json`: the library you last opened, and your theme choice, sort,
  view and File Types choices, one set per photo folder you have opened.
- `fauxcasa.log`: a diagnostic log, capped at about 4 MB. Include a copy
  with names and paths removed when reporting a bug.
- `acl-markers\`: tiny Windows-only marker files recording that the
  decode sandbox's read permission has already been set on a folder, so
  it is not set again on every start.
- `sandbox-worker\`: Windows-only, and only when running from a source
  checkout on a drive that refuses that permission (a Dev Drive, for
  example): a copy of the sandbox worker script, one subfolder per
  content hash of that script, that the sandbox runs from instead. Not
  written by the packaged app.
...
- `stars.json`: the stars you set in Fauxcasa. Kept here, never written
  into your library.
- `config.json`: this library's sort mode, flat or tree view, star
  filter threshold, and the window size and position.
...
certutil -hashfile fauxcasa-0.1.0-rc2-windows-x64.zip SHA256

# Linux/macOS
sha256sum fauxcasa-0.1.0-rc2-linux-x64.tar.gz
```
...
```
gh attestation verify fauxcasa-0.1.0-rc2-windows-x64.zip --repo maphew/fauxcasa
```
...
RAW, PSD, 16-bit TIFF, HEIC/HEIF and video are decoded in-process; on
Linux everything is in-process. If the sandbox cannot start, the app
says so in the status bar and decodes in-process. `FAUXCASA_DECODE_SANDBOX=1` is
