...
you use the optional watched-folders import; see "What Fauxcasa writes on
your computer" below.

## Install and run

You need a 64-bit Windows 10 or 11 PC, or a 64-bit Linux desktop at least
as new as Ubuntu 22.04. There is no Mac version yet.
...
```
sudo apt install libegl1 libgl1 libfontconfig1 libdbus-1-3 libxkbcommon0 \
  libxkbcommon-x11-0 libxcb-cursor0 libxcb-icccm4 libxcb-image0 \
  libxcb-keysyms1 libxcb-render-util0 libxcb-shape0 libxcb-xinerama0
```

On other distributions the package names differ, but the same libraries
(Qt's xcb and xkbcommon helpers, EGL/GL, fontconfig) are what it needs.
You can also run from source instead; see "For developers" in the
[README](../../README.md).
...
time. After that, Fauxcasa remembers the library and starts in a second or
...
- **Star** a photo with Space. Stars you set or remove in Fauxcasa are
  remembered in its own files, never written into your library. Stars
  Picasa recorded show up too.
...
- **Formats**: JPEG, PNG, GIF, BMP, TIFF, WebP and TGA stills; RAW files
  from 16 camera makers; Photoshop PSD; common video formats.
- **A library spread over more than one drive** keeps working when one of
  ...
  again. Adding a second drive or folder to a library is a command-line
  step for now; see the appendix.
...
- Fast scrolling at the smallest thumbnail size can stutter on 4K and
  other high-resolution displays with very large libraries.
- Import notes can be read but not searched, filtered or saved.
- Adding a second drive or folder to a library is a command-line step
  (see the appendix), not something in the menus yet.
...
while a video is playing. The tables are generated from
`apps/desktop-python/keymap.py`.
...
- `config.json`: the library you last opened, sort and view preferences,
  and your File Types choices (per library, keyed by library path).
...
- `catalog.json` (or `catalog.json.zst`): Fauxcasa's list of your photos
  and their Picasa data.
...
watched folders** button, or the `--promote`, `--add-root` and
`--import-picasa-watched` command-line options described in the
appendix):
...
- `.fauxcasa-root`: a tiny marker file, written into each folder that is
  ...
  the folder again if it moves. All four features above write it. If it
...
update checks or usage reports.

## Upgrading from the August preview

If you tried the `tracer-test-v1` preview: extract this version and run
it; it finds your photos again on its own. The preview kept its cache in a
...
- The version: **Help > About** in the app, or run `fauxcasa.exe
  --version` (`fauxcasa-console.exe --version` shows it in a terminal
  window on Windows).
...
download, use the command line, or see how this release was tested.
...
| `--import-picasa-watched <list>` | build a multi-folder library from a list of folders: a text file with one path per line, or the word `registry` to read Picasa's own list on Windows |
